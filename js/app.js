...
var showQuestion = function(question) {
	...
	var result = $('.templates .question').clone(); // * not quite sure what this does
...
var showInspiredQuestion = function(question) {
	
	// clone our result template code

	console.log(question)

	var result = $('.inspiredTemplates .inspiredQuestion').clone(); // * not quite sure what this does
	
	// Set the question properties in result
	var profileImg = result.find('.profileImg img');
	profileImg.attr('src', question.user.profile_image);
	// questionElem.text(question.title);
...
var getInspired = function(tags) {
	...
	.done(function(result){ //this waits for the ajax to return with a succesful promise object
		...
		var searchResults = showSearchResults( tagged , result.items.length);

		$('.search-results').html(searchResults);
		//$.each is a higher order function. It takes an array and a function as an argument.
		//The function is executed once for each item in the array.
		$.each(result.items, function(i, item) {
			var question = showInspiredQuestion(item);
			$('.results').append(question);
		});
	})
	.fail(function(jqXHR, error){ //this waits for the ajax to return with an error promise object
